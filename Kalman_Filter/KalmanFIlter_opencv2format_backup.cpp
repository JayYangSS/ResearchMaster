...
int main()
{
	...
	Mat state (stateNum, 1, CV_32F); //state(x,y,detaX,detaY)
	...
	for(;;)
	{
		...
		Point predictPt = Point( (int)KF.predict().at<float>(0), (int)KF.predict().at<float>(1));
		...
		//draw
		circle(showImg, statePt, 5, CV_RGB(255,0,0),1);//corrected point
