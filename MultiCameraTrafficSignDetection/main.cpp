...
App::App(const Args& s)
{
	...
	{
			args.hit_threshold = signTotal*2;
	}
...
void App::run()
{
    ...
    Size win_size(args.win_width, args.win_width); //(40, 40)
	...
	//declaration of traffic sign HOG descriptors
	vector<float> totalDetector = getTrafficSignDetector("total");
	...
	gpu_hog.setSVMDetector(totalDetector);
    cpu_hog.setSVMDetector(totalDetector);
	...
	for(int i = 0; i < category; ++i)
	{
		...
		HOGDescriptor descriptorTemp(win_size, Size(16, 16), Size(8,8), Size(8, 8), 9,
										1, -1, HOGDescriptor::L2Hys, 0.2, true, HOGDescriptor::DEFAULT_NLEVELS);

		gpu::HOGDescriptor descriptorTempGpu(win_size, Size(16, 16), Size(8,8), Size(8, 8), 9,
...
//*********************************tracking begin************************************************//
			...
			else
			{
				...
				for( size_t i = 0; i < found_filters.size(); ++i)
				{
					Rect roi;	//extract the region of interest from the orignal image.
					//Rect rs = found_filters[i];
					roi.x += found_filters[i].x - found_filters[i].width*0.2;
					roi.width = found_filters[i].width*1.4;
					roi.y += found_filters[i].y - found_filters[i].height*0.2;
					roi.height = found_filters[i].height*1.4;
					if(roi.x < 0)
						roi.x = 0;
					if(roi.y < 0)
						roi.y = 0;
					if((roi.x + roi.width) > imgs.cols)
						roi.width = imgs.cols - roi.x;
					if((roi.y + roi.height) > imgs.rows)
						roi.height = imgs.rows - roi.y;
					if ((roi.width <40)||(roi.height < 40))	//the effect or role??
						continue;

					//flags
					bool flagClassification[20] = {0};
					//detected traffic sign classification
					for(int j = 0; j < category; ++j)
					{
						...
						else
						{
							switch(j)
							{
								case 0:
									categoryClassified = "40";
									gpuTrafficSignClassifier[j].detectMultiScale(imgROIGpu, foundClassified, 8.1, 
																Size(8,8), Size(0,0), 1.05, 2); //threshold						
									break;
								case 1:
									categoryClassified = "50";
									gpuTrafficSignClassifier[j].detectMultiScale(imgROIGpu, foundClassified, 7.5, 
																Size(8,8), Size(0,0), 1.05, 2); //threshold						
									break;
								case 2:
									categoryClassified = "stop";
									gpuTrafficSignClassifier[j].detectMultiScale(imgROIGpu, foundClassified, 6.8, 
																Size(8,8), Size(0,0), 1.05, 2); //threshold						
									break;
				...
				for( size_t i = 0; i < foundtempl.size(); ++i)
				{
					Rect roi;	//extract the region of interest from the orignal image.
					//Rect rs = found_filters[i];
					roi.x += foundtempl[i].x - foundtempl[i].width*0.2;
					roi.width = foundtempl[i].width*1.4;
					roi.y += foundtempl[i].y - foundtempl[i].height*0.2;
					roi.height = foundtempl[i].height*1.4;
					if(roi.x < 0)
						roi.x = 0;
					if(roi.y < 0)
						roi.y = 0;
					if((roi.x + roi.width) > imgl.cols)
						roi.width = imgl.cols - roi.x;
					if((roi.y + roi.height) > imgl.rows)
						roi.height = imgl.rows - roi.y;
					if ((roi.width <40)||(roi.height < 40))	//the effect or role??
						continue;
