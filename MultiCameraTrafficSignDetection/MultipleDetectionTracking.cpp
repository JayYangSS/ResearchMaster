...
App::App(const Args& s)
{
    ...
    if (args.hit_threshold_auto)
	{
		...
		if(args.win_width == 48)
			args.hit_threshold = threshold48*2 + 1.7;//1.59
		if(args.win_width == 56)
			args.hit_threshold = threshold56*2;
		if(args.win_width == 64)
			args.hit_threshold = threshold64*2 + 0.85;//0.85
...
void App::run()
{
	...
	gpu_hog.setSVMDetector(getThisObjectDetector(args.win_width));
    cpu_hog.setSVMDetector(getThisObjectDetector(args.win_width));


	//declaration of pedestrian HOG descriptors
	gpu::HOGDescriptor gpuHOGSign(signSize, Size(16, 16), Size(8, 8), Size(8, 8), 9,
                                   cv::gpu::HOGDescriptor::DEFAULT_WIN_SIGMA, 0.2, gamma_corr,
                                   cv::gpu::HOGDescriptor::DEFAULT_NLEVELS);
    cv::HOGDescriptor cpuHOGSign(signSize, Size(16, 16), Size(8, 8), Size(8, 8), 9, 1, -1,
                              HOGDescriptor::L2Hys, 0.2, gamma_corr, cv::HOGDescriptor::DEFAULT_NLEVELS);

	gpuHOGSign.setSVMDetector(getTrafficSignDetector("total"));
    cpuHOGSign.setSVMDetector(getTrafficSignDetector("total"));

	vector<HOGDescriptor> cpuTrafficSignClassifier;
	vector<gpu::HOGDescriptor> gpuTrafficSignClassifier;
	for(int i = 0; i < category; ++i)
	{

		HOGDescriptor descriptorTemp(signSize, Size(16, 16), Size(8,8), Size(8, 8), 9,
										1, -1, HOGDescriptor::L2Hys, 0.2, true, HOGDescriptor::DEFAULT_NLEVELS);

		gpu::HOGDescriptor descriptorTempGpu(signSize, Size(16, 16), Size(8,8), Size(8, 8), 9,
		...
		switch(i)
		{
			...
			case 0:
				...
				descriptorTempGpu.setSVMDetector( getTrafficSignDetector("40") );
				cpuTrafficSignClassifier.push_back(descriptorTemp);
				gpuTrafficSignClassifier.push_back(descriptorTempGpu);
				break;
			case 1:
				descriptorTemp.setSVMDetector( getTrafficSignDetector("50") );//50-7.5
				descriptorTempGpu.setSVMDetector( getTrafficSignDetector("50") );
				cpuTrafficSignClassifier.push_back(descriptorTemp);
				gpuTrafficSignClassifier.push_back(descriptorTempGpu);
				break;
			case 2:
				descriptorTemp.setSVMDetector( getTrafficSignDetector("stop") );//stop-6.44
				descriptorTempGpu.setSVMDetector( getTrafficSignDetector("stop") );
				cpuTrafficSignClassifier.push_back(descriptorTemp);
				gpuTrafficSignClassifier.push_back(descriptorTempGpu);
				break;
			//case 3:
    ...
    while (running)
    {
        ...
        while (running && !frames.empty() && !framel.empty())
        {
			...
			vector<Rect> foundtempl, foundtemplns;//foundtemps are not used, foundtempl is used for tracking 
			
			vector<Rect> foundSigns, foundSignl, foundSignFilters, foundSignFilterl;
			vector<Rect> foundSignTempl, foundSignTemplns;//foundtemps are not used, foundtempl is used for tracking 
            ...
            if (use_gpu)
            {
				...
				if(detectSign)
				{
					gpuHOGSign.detectMultiScale(gpu_imgs, foundSigns, 9.0, win_stride,
											 Size(0, 0), scale, gr_threshold);
					gpuHOGSign.detectMultiScale(gpu_imgl, foundSignl, 9.0, win_stride,
											 Size(0, 0), scale, gr_threshold);
					//cout << "foundSigns.size(): " << foundSigns.size() << endl;
					//cout << "foundSignl.size(): " << foundSignl.size() << endl;
				}
				detectionModel = "GPU";
            }
            else 
			...
			if(!use_SIFT)
			{
				...
				for ( size_t i = 0; i < found_filterl.size(); ++i )			//eleminiate same detection results
				{
					...
					size_t j = 200;
					for ( j = 0; j < found_filters.size(); ++j )
					{
						Rect rs = found_filters[j];			

						//eliminate the reduplicated results for the same person,if there are some in common, 
						//if the long recs in the short recs, eliminate them
						if( (( rls.x + rls.width/2 ) > rs.x )
							&& (( rls.x + rls.width/2 ) < ( rs.x + rs.width ))
							&& (( rls.y + rls.height/2 ) > rs.y)
							&& (( rls.y + rls.height/2 ) < ( rs.y + rs.height )) )
							break;
						//if the abs of the left-upper corner of the long recs and short recs less than 30, eliminate them
						else if ( (abs(rls.x-rs.x) <= 30) && (abs(rls.y-rs.y) <= 30) )
							break;
						//if the abs of the left-lower corner of the long recs and short recs less than 30, eliminate them
						else if ( (abs(rls.y + rls.height - rs.y - rs.height) <= 30) && (abs(rls.x + rls.height - rs.x - rs.height) <= 30) )
							break;
						//if the abs of the right-upper corner of the long recs and short recs less than 30, eliminate them
						else if ( (abs(rls.y + rls.width - rs.y - rs.width) <= 30) && (abs(rls.x + rls.width - rs.x - rs.width) <= 30) )
							break;
						//if the abs of the right-lower corner of the long recs and short recs less than 30, eliminate them
						else if ( (abs(rls.br().x - rs.br().x) <= 30) && (rls.br().y - rs.br().y) <= 30 )
							break;
					}
					//countElimination += 1;
				...
				if(detectSign)
				{
					...
					for ( size_t i = 0; i < foundSignFilterl.size(); ++i )			//eleminiate same detection results
					{
						...
						size_t j = 200;
						for ( j = 0; j < foundSignFilters.size(); ++j )
						{
							Rect rsSign = foundSignFilters[j];			
...
//*********************************traffic sign classification************************************************//
			...
			if(detectSign)
			{
				...
				for( size_t i = 0; i < foundSignFilterl.size(); ++i)
				{
					rectangle(img_to_showl, foundSignFilterl[i], CV_RGB(0, 255, 255), 2);
				}
				...
				for( size_t i = 0; i < foundSignTemplns.size(); ++i)
				{
					...
					Rect roi;	
					roi.x += foundSignTemplns[i].x - foundSignTemplns[i].width*0.2;
					roi.width = foundSignTemplns[i].width*1.4;
					roi.y += foundSignTemplns[i].y - foundSignTemplns[i].height*0.2;
					roi.height = foundSignTemplns[i].height*1.4;
					if(roi.x < 0)
						roi.x = 0;
					if(roi.y < 0)
						roi.y = 0;
					...
					for(int j = 0; j < category; ++j)
					{
						...
						if(!use_gpu)
						{
							switch(i)
							{
								case 0:
									...
									cpuTrafficSignClassifier[i].detectMultiScale(imgROI, foundClassified, 8.1, 
																Size(8,8), Size(0,0), 1.05, 2); //threshold						
									break;
								case 1:
									categoryClassified = "50";
									cpuTrafficSignClassifier[i].detectMultiScale(imgROI, foundClassified, 7.5, 
																Size(8,8), Size(0,0), 1.05, 2); //threshold						
									break;
								case 2:
									categoryClassified = "stop";
									cpuTrafficSignClassifier[i].detectMultiScale(imgROI, foundClassified, 6.8, 
																Size(8,8), Size(0,0), 1.05, 2); //threshold						
									break;
						...
						else
						{
							switch(i)
							{
								case 0:
									categoryClassified = "40";
									gpuTrafficSignClassifier[i].detectMultiScale(imgROIGpu, foundClassified, 8.1, 
																Size(8,8), Size(0,0), 1.05, 2); //threshold						
									break;
								case 1:
									categoryClassified = "50";
									gpuTrafficSignClassifier[i].detectMultiScale(imgROIGpu, foundClassified, 7.5, 
																Size(8,8), Size(0,0), 1.05, 2); //threshold						
									break;
								case 2:
									categoryClassified = "stop";
									gpuTrafficSignClassifier[i].detectMultiScale(imgROIGpu, foundClassified, 6.8, 
																Size(8,8), Size(0,0), 1.05, 2); //threshold						
									break;
...
//*********************************tracking begin************************************************//
			if( btracking )
			{
				if(!firstframe)												//not the initialization step
				{
					...
					size_t sizeS = KFTvectors.size();					//get the current vector size
					cout<<"\n\n"<<endl;
					cout<<"sizeS = KFTvectors.size(): "<< sizeS <<endl;
					size_t sizeL = KFTvectorl.size();
					cout<<"sizeL = KFTvectorl.size(): "<< sizeL <<endl;

					for(size_t i = 0; i < found_filters.size(); ++i)	//put new detection results into KalmanFilters---short focal length image					
					{
						cout<<"\n\n"<<endl;
						...
						for( j = 0; j < sizeS; ++j)					//cycle for the original KFTvector
						{	
							cout<< "begin cycle for the original KFTvectors: j =  "<<j<<endl;

							int t = KFTvectors[j].predictRectVector.size() - 1 ;
							if( t < 0)
								continue;
							Rect temp = KFTvectors[j].predictRectVector[t];	
						...
						cout<< "after cycle for the original KFTvectors, j = "<<j<<endl;
						cout<< "after cycle for the original KFTvectors, KFTvectors.size = "<<sizeS<<endl;

						if( j == sizeS )
						{	
							cout<< "befroe KFTvectors.push_back(KFTtempl): "<< KFTvectors.size()<<endl;
							...
							cout<< "after KFTvectors.push_back(KFTtempl): "<< KFTvectors.size()<<"\n"<<endl;
						}

					}
					
					for(size_t i = 0; i < foundtempl.size(); ++i)					//put new detection results into KalmanFilters---long local length image
					{
						cout<<"\n\n"<<endl;
						...
						size_t j;
						for( j = 0; j < sizeL; ++j)								//cycle for the original KFTvector
						{
							cout<< "cycle for the original KFTvectorl: j =  "<<j<<endl;

							int t = KFTvectorl[j].predictRectVector.size() - 1;	
							if( t < 0)
								continue;
							Rect temp = KFTvectorl[j].predictRectVector[t];	
							//if the measurement result's center in the rectangle of statepost,update it
						...
						cout<< "after cycle for the original KFTvectorl, j = "<<j<<endl;
						cout<< "after cycle for the original KFTvectorl, KFTvectorl.size = "<<sizeL<<endl;

						if(j == sizeL)									//put new detection in to tracking, even if the the KFTvector is empty
						{
							...
							cout<< "after KFTvectorl.push_back(KFTtempl): "<<"\n"<< KFTvectorl.size()<<endl;
						}
					...
					for(size_t j = 0; j < KFTvectors.size(); ++j)
					{
						...
						int t = KFTvectors[j].predictRectVector.size() - 1;			//t == the newest Rect vector number
						if( t < 0)
							continue;
						Rect temp = KFTvectors[j].predictRectVector[t];							//temp == the neweset Rect in predictRectVector
						size_t i;
					...
					{
						cout << "KFTvectorl.size() after new detection reults in, " << "total: "<< KFTvectorl.size() << "KFTvectorl[ " << j<<"]"<<endl;

						KalmanFilterTracker KFTtemplts;
						int t = KFTvectorl[j].predictRectVector.size() - 1;			//t == the newest Rect vector number
						if( t < 0)
							continue;
						Rect temp = KFTvectorl[j].predictRectVector[t];							//temp == the neweset Rect in predictRectVector
						size_t i;
						for( i = 0; i < foundtempl.size(); ++i)
						{
								...
								&& (( rl.y + rl.height/2 ) < ( temp.y + temp.height ))	)
							{
								...
								for( size_t t = 0; t < KFTvectorl[j].predictRectVector.size(); ++t)
								{
									Rect temp;
									temp.x = KFTvectorl[j].predictRectVector[t].x*ratio + resx + tempx;
									temp.y = KFTvectorl[j].predictRectVector[t].y*ratio + resy + tempy;
									temp.width = KFTvectorl[j].predictRectVector[t].width*ratio;
									temp.height = KFTvectorl[j].predictRectVector[t].height*ratio;
									KFTtemplts.predictRectVector.push_back( temp ) ;						
								}
					...
					for(size_t i = 0; i < KFTvectors.size(); ++i)
					{
						...
						KFTvectors.at(i).changeStatus(img_to_shows);						//danger or warning status
					...
					for(size_t i = 0; i < KFTvectorl.size(); ++i)
					{
						...
						KFTvectorl.at(i).changeStatus(img_to_showl);
					...
					for(size_t i = 0; i < KFTvectorlns.size(); ++i)
					{
						...
						KFTvectorlns.at(i).changeStatus(img_combine);
						KFTvectorlns.at(i).drawPredictLine(img_combine);
						KFTvectorlns.at(i).putStatusText(img_combine);
						
						int temp = KFTvectorlns[i].predictRectVector.empty() ? 0 : KFTvectorlns[i].predictRectVector.size() - 1;;
						if( temp > 0)
						{

							Rect rls = KFTvectorlns[i].predictRectVector.at(temp);
							positionT.str("");
							areaT.str("");
							diameterT.str("");
